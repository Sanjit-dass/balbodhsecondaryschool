    ...
    <div className="w-full max-w-full space-y-4">
      ...
      {loading ? (
        ...
        (marks.length === 0 ? (
          ...
          <>
            <div className="overflow-x-auto bg-white shadow rounded-lg md:block hidden">
            ...
            <div className="space-y-3 md:hidden">
              {marks
                ...
                .map((mark, idx) => {
                  const total = (Number(mark.theoryMarks) || 0) + (Number(mark.practicalMarks) || 0);
                  const maxTheory = Number(mark.maxTheoryMarks) || 50;
                  const maxPractical = Number(mark.maxPracticalMarks) || 50;
                  const theoryPassThreshold = Number(theoryPassMarks) || 20;
                  const practicalPassThreshold = Number(practicalPassMarks) || 20;
                  const theory = mark.theoryMarks !== '' && mark.theoryMarks !== null ? Number(mark.theoryMarks) : 0;
                  const practical = mark.practicalMarks !== '' && mark.practicalMarks !== null ? Number(mark.practicalMarks) : 0;
                  ...
                  return (
                    <div key={idx} className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <div className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">{mark.rollNumber || '-'}</div>
                          <div className="text-sm font-semibold text-slate-900">{mark.studentName}</div>
                        </div>
                        <div className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">{status}</div>
                      </div>

                      <div className="mt-3 grid grid-cols-2 gap-2">
                        <div className="rounded-lg bg-slate-50 p-2">
                          <div className="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-500">Theory</div>
                          <input
                            type="text"
                            placeholder="T"
                            value={mark.theoryMarks}
                            onChange={e => updateMark(idx, 'theoryMarks', e.target.value)}
                            className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5 text-center text-sm"
                          />
                        </div>
                        <div className="rounded-lg bg-slate-50 p-2">
                          <div className="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-500">Practical</div>
                          <input
                            type="text"
                            placeholder="P"
                            value={mark.practicalMarks}
                        ...
                        <div className="rounded-lg bg-slate-50 p-2">
                          <div className="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-500">Max Theory</div>
                          <input
                            type="number"
                            placeholder="50"
                            value={mark.maxTheoryMarks}
                        ...
                        <div className="rounded-lg bg-slate-50 p-2">
                          <div className="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-500">Max Practical</div>
                          <input
                            type="number"
                            placeholder="50"
                            value={mark.maxPracticalMarks}
                      ...
                      <div className="mt-3 flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2 text-sm">
                        <span className="text-slate-600">Total: <span className="font-semibold text-slate-900">{total}</span></span>
                        <button
                          type="button"
                          onClick={() => saveMark(mark)}
                          disabled={saving}
        ...
        <div className="w-full max-w-full rounded-lg border border-slate-200 bg-white shadow-sm">
          ...
          <div className="space-y-3 p-3 md:hidden">
            {marksMatrix
              ...
              .map((row, ridx) => (
                <div key={ridx} className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <div className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">{row.rollNumber || '-'}</div>
                      <div className="text-sm font-semibold text-slate-900">{row.studentName}</div>
                    </div>
                    <div className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">
                      Total: {row.subjects.reduce((sum, sub) => {
                        const theory = sub.theoryMarks !== '' && sub.theoryMarks !== null ? Number(sub.theoryMarks) : 0;
                        const practical = sub.practicalMarks !== '' && sub.practicalMarks !== null ? Number(sub.practicalMarks) : 0;
                        return sum + (isNaN(theory) ? 0 : theory) + (isNaN(practical) ? 0 : practical);
                      }, 0)}
                    </div>
                  </div>
                  ...
                  <div className="mt-3 space-y-2">
                    {row.subjects.map((sub, sidx) => (
                      <div key={sub.subjectId} className="rounded-lg bg-slate-50 p-2">
                        <div className="mb-2 text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">{subjects.find(subject => subject._id === sub.subjectId)?.name || 'Subject'}</div>
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            ...
                            <input
                              ...
                              placeholder="T"
                              value={sub.theoryMarks}
                              onChange={e => updateMatrixMark(ridx, sidx, 'theoryMarks', e.target.value)}
                              className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5 text-center text-sm"
                            />
                          </div>
                          <div>
                            ...
                            <input
                              ...
                              placeholder="P"
                              value={sub.practicalMarks}
                              onChange={e => updateMatrixMark(ridx, sidx, 'practicalMarks', e.target.value)}
                              className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5 text-center text-sm"
                            />
                          </div>
                        </div>
