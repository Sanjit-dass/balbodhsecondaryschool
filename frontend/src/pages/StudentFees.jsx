              ...
              <div className="flex items-center gap-3">
                <button 
                  ...
                  onClick={async ()=>{
                    ...
                    try {
                      const payload = { name: claimName, className: claimClass, rollNumber: claimRoll };
                      const res = await api.post('/fees/student/claim', payload);
                      if (res.data && res.data.student) {
                        const s = res.data.student;
                        setSelectedStudent(s);
                        setClaimSuccess('Student record claimed successfully. Redirecting...');
                        setTimeout(() => {
                          window.location.href = `/student/fees`;
                        }, 1500);
                      } else if (res.data && res.data.matches) {
                        setClaimMatches(res.data.matches);
                        setClaimError('Multiple matches found — please select the correct student below.');
                      } else {
                        setClaimError(res.data?.message || 'No matching student found.');
                      }
                    } catch (err) {
                      console.error(err);
              ...
              {claimMatches ? (
                <div>
                  ...
                  <div className="space-y-2">
                    {claimMatches.map(m => (
                      <div key={m.id} className="flex items-center justify-between border border-slate-200 p-3 rounded">
                        <div>
                        ...
                        <button 
                          onClick={async ()=>{
                            ...
                            try {
                              const res = await api.post('/fees/student/claim', { studentId: m.id });
                              if (res.data && res.data.student) {
                                ...
                                setClaimError(res.data?.message || 'Unable to claim selected student');
                              }
                            } catch (err) {
                              console.error(err);
                              setClaimError(err?.response?.data?.message || err.message || 'Claim failed');
                          ...
                          className="px-3 py-1 bg-emerald-600 text-white rounded text-sm font-semibold hover:bg-emerald-700 transition"
                        >
                          Select
                        </button>
                      </div>
