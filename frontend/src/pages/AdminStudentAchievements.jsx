...
export default function AdminStudentAchievements(){
  ...
  // Lock body scroll when modal/form is open (prevents mobile overscroll); restore on close
  useEffect(() => {
  ...
  function emptyForm(){ return { studentName:'', class:'1', title:'', category:'Academic', shortDescription:'', description:'', achievementDate:'', achievementLevel:'School Level', award:'', statistics:[], photos:[], status:'published', displayOrder: (achievements.length+1), _newFiles:[], _coverIndex:0 }; }
  ...
  function openEdit(a){
    ...
    if (a.coverPhoto && Array.isArray(a.photos)){
      const idx = (a.photos||[]).findIndex(p=> String(p._id) === String(a.coverPhoto)); if (idx>=0) coverIndex = idx;
    }
    setEditing({...a, statistics:stats, _newFiles:[], _coverIndex:coverIndex, achievementDate: formatDate(a.achievementDate)});
    setShowForm(true);
  }

  function closeForm(){ try{ if (editing && Array.isArray(editing._newFiles)) editing._newFiles.forEach(f=> { try{ if (f.previewUrl) URL.revokeObjectURL(f.previewUrl); }catch(e){} }); }catch(e){} setShowForm(false); setEditing(null); setSaveError(''); setSaving(false); }
  ...
  function handleFileSelect(e){ const files = Array.from(e.target.files||[]).map(f=> Object.assign(f, { previewUrl: URL.createObjectURL(f) })); setEditing(prev=> ({ ...prev, _newFiles: [...(prev._newFiles||[]), ...files] })); }
  function removeNewFile(i){ setEditing(prev=> { const toRevoke = prev._newFiles && prev._newFiles[i]; try{ if (toRevoke && toRevoke.previewUrl) URL.revokeObjectURL(toRevoke.previewUrl); }catch(e){} return { ...prev, _newFiles: prev._newFiles.filter((_,idx)=> idx!==i) }; }); }
  ...
  async function save(){
    try{
      ...
      if (isNew && (!hasFiles && (!editing.photos || editing.photos.length===0))){ alert('Cover photo is required. Please upload at least one photo and set it as cover.'); setSaving(false); return; }

      let res;
      if (isNew || hasFiles){
        const fd = new FormData();
        fd.append('studentName', editing.studentName||'');
        fd.append('class', editing.class||'');
        ...
        fd.append('achievementLevel', editing.achievementLevel||'School Level');
        fd.append('award', editing.award||'');
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <div className="text-sm text-slate-600">Manage student achievements, recognition records, scholarships, competition winners, awards, and success stories.</div>
          </div>
          <div>
          ...
          <>
                  ...
                  {achievements.map(a=>{
                    ...
                    return (
                      <tr key={a._id} className="border-t">
                        ...
                        <td className="px-4 py-3 text-left"><div className="font-semibold">{a.studentName||'—'}</div></td>
                        <td className="px-4 py-3">{a.title}</td>
                        <td className="px-4 py-3">{a.class||''}</td>
            ...
            {showForm && editing && (
              <div className="fixed inset-0 z-50 flex items-start justify-center pt-20">
                ...
                <div className="relative z-10 w-full max-w-5xl bg-white rounded shadow-lg p-6" style={{ maxHeight: 'calc(100vh - 160px)', overflowY: 'auto' }}>
                  <div className="md:flex gap-6">
                    <div className="md:w-3/4 space-y-4">
                      <div>
                        <label className="block font-semibold">Student Name</label>
                        <input className="w-full border p-2 rounded" value={editing.studentName||''} onChange={(e)=> setEditing(prev=> ({ ...prev, studentName: e.target.value }))} />
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                          ...
                          <ResponsiveSelect
                            value={editing.class||'1'}
                            onChange={(v)=> setEditing(prev=> ({ ...prev, class: v }))}
                            options={[
                              { value: 'Nursery', label: 'Nursery' },
                              { value: 'LKG', label: 'LKG' },
                              { value: 'UKG', label: 'UKG' },
                              ...Array.from({length:10}).map((_,i)=> ({ value: String(i+1), label: String(i+1) }))
                            ]}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <div>
                          <label className="block font-semibold">Achievement Level</label>
                          <select className="w-full border p-2 rounded" value={editing.achievementLevel||'School Level'} onChange={(e)=> setEditing(prev=> ({ ...prev, achievementLevel: e.target.value }))}>
                            <option>School Level</option>
                            <option>District Level</option>
                            <option>Provincial Level</option>
                      ...
                      <div>
                        <label className="block font-semibold">Award / Recognition</label>
                        <input className="w-full border p-2 rounded" value={editing.award||''} onChange={(e)=> setEditing(prev=> ({ ...prev, award: e.target.value }))} />
                      </div>
                        ...
                        <div className="mt-2">
                          <table className="w-full text-sm border">
                            <thead className="bg-slate-50 text-slate-700 text-left">
                              <tr><th className="px-2 py-1">Statistic</th><th className="px-2 py-1">Value</th><th className="px-2 py-1">Actions</th></tr>
                            </thead>
                            <tbody>
                              {(editing.statistics||[]).map((s,idx)=> (
                                <tr key={idx} className="border-t"><td className="px-2 py-1">{s.label}</td><td className="px-2 py-1">{s.value}</td><td className="px-2 py-1"><div className="flex gap-2"><button onClick={()=>{ const label=prompt('Statistic name', s.label); if(!label) return; const val=prompt('Value', s.value); if(val==null) return; editStat(idx,label,val);} } className="text-xs px-2 py-1 border rounded">Edit</button><button onClick={()=> removeStat(idx)} className="text-xs px-2 py-1 border rounded text-red-600">Delete</button></div></td></tr>
                              ))}
                            </tbody>
                          </table>
                        </div>
                      ...
                      <div>
                        <div className="font-semibold mb-2">Cover Photo (required) & Gallery</div>
                        <div className="border-dashed border-2 border-slate-300 p-4 rounded text-center text-sm">
                          <div>Upload cover and gallery images below.</div>
                          ...
                          {(editing._newFiles||[]).map((f,i)=> (
                            <div key={i} className="border rounded overflow-hidden p-1">
                              <div className="relative">
                                ...
                                {editing._coverIndex===i && <div className="absolute left-1 top-1 bg-blue-600 text-white text-xs px-2 py-0.5 rounded">Cover</div>}
                              </div>
                          ...
                          {(editing.photos||[]).map((p,i)=> (
                            <div key={i} className="border rounded overflow-hidden p-1">
                              <div className="relative">
                                ...
                                {editing.photos && editing.photos[0] && editing.photos[0].url === p.url && (<div className="absolute left-1 top-1 bg-blue-600 text-white text-xs px-2 py-0.5 rounded">Cover</div>)}
                              </div>
                              <div className="flex items-center gap-1 mt-1 flex-wrap">
                                <button disabled={i===0} onClick={()=> { const pArr = [...(editing.photos||[])]; const item = pArr.splice(i,1)[0]; pArr.splice(i-1,0,item); setEditing(prev=> ({ ...prev, photos: pArr })); }} className="px-2 py-1 text-xs border rounded">↑</button>
                                <button disabled={i===(editing.photos||[]).length-1} onClick={()=> { const pArr = [...(editing.photos||[])]; const item = pArr.splice(i,1)[0]; pArr.splice(i+1,0,item); setEditing(prev=> ({ ...prev, photos: pArr })); }} className="px-2 py-1 text-xs border rounded">↓</button>
                                <button onClick={()=> deletePhoto(i)} className="px-2 py-1 text-xs border rounded text-red-600">Delete</button>
                    ...
                    <div className="md:w-1/4">
                      <div className="sticky top-5 bg-white p-3 rounded border">
                          ...
                          <div className="mt-2 border rounded p-3 bg-white">
                            <div className="flex gap-3">
                              <div className="w-1/3 h-28 bg-slate-100 flex items-center justify-center overflow-hidden rounded">
                                {(() => { const newFilesLen = editing._newFiles ? editing._newFiles.length : 0; if (editing._coverIndex !== undefined && editing._coverIndex < newFilesLen){ const f = editing._newFiles[editing._coverIndex]; return <img src={f.previewUrl||''} alt="cover" className="h-full w-full object-cover"/>; } const photoIdx = editing._coverIndex !== undefined ? editing._coverIndex - newFilesLen : 0; if (editing.photos && editing.photos[photoIdx]) return <img src={editing.photos[photoIdx].url} alt="cover" className="h-full w-full object-cover" />; return <div className="text-sm text-slate-500">No cover</div>; })()}
                              </div>
                              <div className="flex-1">
                                <div className="font-semibold text-lg">{editing.title || 'Untitled'}</div>
                                <div className="text-sm text-slate-600 mt-1">{editing.studentName || ''} • {editing.class || ''}</div>
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={save} disabled={saving} className={`flex-1 px-3 py-2 text-white rounded ${saving? 'bg-slate-400':'bg-blue-600'}`}>{saving? 'Saving…':'Save'}</button>
                        </div>
                      </div>
                    </div>
                  </div>
