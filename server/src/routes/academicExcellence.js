...
router.get('/', async (req, res) => {
  try {
    const items = await AcademicExcellence.find({ status: { $ne: 'hidden' } }).sort({ displayOrder: 1, createdAt: -1 }).lean();
...
router.post('/', auth, roles(['superadmin','principal','admin']), upload.array('photos'), async (req, res) => {
  try {
    ...
    const item = new AcademicExcellence({ title, shortDescription, description, statistics, displayOrder, status, createdBy: req.user.id || req.user._id });
...
router.patch('/:id', auth, roles(['superadmin','principal','admin']), async (req, res) => {
  try {
    ...
    const { title, description, statistics, displayOrder, status, shortDescription } = req.body;
